...
def PieChart(clust, threshold):
    ...
    effct = 0
    explode=[0]
    for i in xrange(int(np.max(chart))):
...
np.savetxt(traj[:-4]+'_angles4_clustCentersFrameNo.txt', clustCentersFrameNo, fmt='%d')
t = md.load(traj, top=topfile)
#check results:
#print(clustCentersFrameNo)
print('cluster no., centroid, nb_elemnt, pourcentage')
for i in xrange(len(clustCenters)):
    ...
    if pourcentage > 5:
        ...
        t[clustCentersFrameNo[i]].save_pdb("centroid_clust"+str(i)+".pdb")
